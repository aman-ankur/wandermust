...
logger = logging.getLogger("wandermust.weather")
...
def get_weather_for_window(latitude, longitude, start_date, end_date):
    ...
    logger.info(f"Weather API: lat={latitude:.2f} lon={longitude:.2f}, {start_date}→{end_date}, {'climate' if is_future else 'historical'}")
    params = {
    ...
    logger.info(f"Weather result: temp={result['avg_temp']}°C, rain_days={result['rain_days']}, humidity={result['avg_humidity']}%")
    return result
